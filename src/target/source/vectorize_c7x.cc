...
/*!
 * \file vectorize_c7x.cc
 */
// C7x Loop vectorizer, adapted from vectorize_loop.cc
...
class C7xVectorizer : public StmtMutator, public ExprFunctor<PrimExpr(const PrimExpr&)> {
 public:
  using ExprFunctor::VisitExpr;
  using StmtMutator::operator();

  C7xVectorizer(Var var, int var_lanes) : var_(var), var_lanes_(var_lanes) {
  ...
  PrimExpr VisitExpr_(const MulNode* op) final {
    ...
    if (a.same_as(op->a) && b.same_as(op->b)) {
      ...
      // distribute mul into ramp
      // ramp(base, stride, lanes) * k ==> ramp(base*k, stride*k, lanes)
  ...
  PrimExpr VisitExpr_(const CallNode* op) final {
    ...
    if (op->op.same_as(builtin::call_extern()) && 
        Downcast<StringImm>(op->args[0])->value == "c7x_stream_access") {
      PrimExpr index = this->VisitExpr(op->args[4]);
      return Call(op->dtype.with_lanes(index.dtype().lanes()), op->op, { op->args[0], op->args[1], op->args[2], op->args[3], index } );
    }
  ...
  PrimExpr VisitExpr_(const LoadNode* op) final {
    ...
    if (index.same_as(op->index) && pred.same_as(op->predicate)) {
      ...
      PrimExpr index_ramp;
      DataType dt = op->dtype.with_lanes(lanes);
      #if 0
      const RampNode* i_ramp = index.as<RampNode>();
      if (i_ramp)
         index_ramp = Call(i_ramp->dtype, builtin::call_extern(),
                           {StringImm("c7x_se"), GetRef<PrimExpr>(i_ramp)});
      else
      #endif
         index_ramp = BroadcastTo(index, lanes);


      return Load(dt, op->buffer_var, index_ramp, BroadcastTo(pred, lanes));
    }
  }
  // Let
  ...
  #else
  Stmt VisitStmt_(const AllocateNode* op) final {
    LOG(FATAL) << "Can't handle Allocate statement in C7x vectorizer";
    return Stmt();
  }
  #endif
 ...
 private:
  ...
  // used to vectorize call arguments
  ...
  PrimExpr AddSubVec(const T* op, FCompute fcompute) {
    ...
    if (a.same_as(op->a) && b.same_as(op->b)) {
      ...
      // Distribute add/sub into ramp: 
      // ramp(base, stride, lanes) + k ==> ramp(base+k, stride, lanes)
      int lanes = std::max(a.dtype().lanes(), b.dtype().lanes());
      if (lanes != 1) {
        const RampNode* b_ramp = b.as<RampNode>();
        const RampNode* a_ramp = a.as<RampNode>();
...
class C7xLoopVectorizer : public StmtMutator {
 public:
  Stmt VisitStmt_(const ForNode* op) final {
    if (op->kind == ForKind::kVectorized) {
      printf("c7x vectorizer\n");
      ICHECK(is_zero(op->min));
      auto* extent_as_int = op->extent.as<IntImmNode>();
      if (!extent_as_int || extent_as_int->value < 1) {
        LOG(FATAL) << "Failed to vectorize loop with extent " << op->extent;
      }
      return C7xVectorizer(op->loop_var, static_cast<int>(extent_as_int->value))(op->body);
    } else {
...
//static Stmt VectorizeLoop(Stmt stmt) { return LoopVectorizer()(std::move(stmt)); }

namespace transform {
namespace c7x {

Pass C7xVectorizeLoop() {
  auto pass_func = [=](PrimFunc f, IRModule m, PassContext ctx) {
    auto* n = f.CopyOnWrite();
    n->body = C7xLoopVectorizer()(std::move(n->body));
    return f;
  };
  return CreatePrimFuncPass(pass_func, 0, "tir.VectorizeC7x", {});
}

TVM_REGISTER_GLOBAL("tir.c7x.VectorizeC7x").set_body_typed(C7xVectorizeLoop);
