...
function build_tvm {
    ...
    mkdir $BUILD_DIR
    cd $BUILD_DIR
    cmake -DCMAKE_EXPORT_COMPILE_COMMANDS=YES -DUSE_MICRO=ON -DUSE_SORT=ON -DUSE_TIDL=ON -DUSE_LLVM="${LLVM_CONFIG} --link-static" -DHIDE_PRIVATE_SYMBOLS=ON -DUSE_TIDL_RT_PATH=$(ls -d ${PSDKR_PATH}/tidl_j7*/arm-tidl/rt) -DUSE_TIDL_PSDKR_PATH=${PSDKR_PATH} -DUSE_CGT7X_ROOT=${TVM_DEPS_PATH}/ti-cgt-c7000_4.1.0.LTS ..
    make -j$(nproc)
