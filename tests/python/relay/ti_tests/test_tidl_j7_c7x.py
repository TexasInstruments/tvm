...
def test_tidl_mxnet(model_name, img_file_list):
    ...
    input_shape = (1, 3, 224, 224)
    input_data_list = [ load_image(1, img_file, [256, 256], [224, 224],
                            [128, 128, 128], [0.0078125, 0.0078125, 0.0078125], True,
                            model_name.endswith('_quant'))
                            for img_file in img_file_list ]
    if input_data_list[0].shape != input_shape:
        sys.exit("Input data shape is not correct!")
    ...
    mxnet_model_name = model_name[:-4] if model_name.endswith('_c7x') else model_name
    model = model_zoo.get_model(mxnet_model_name, pretrained=True)
